...
(async() => {
  ...
  const model = await fs.readFileSync('test2.json');
  ...
  Object.keys(modelJson).forEach((key) => {
    modelJson[key] = tf.tensor(modelJson[key], [modelJson[key].length / 1024, 1024]);
  })

  classifier.setClassifierDataset(modelJson);
  ...
  //   await fs.writeFileSync("test.json", json)
  // }
